...
| `forward(target: torch.Tensor, content_features: Dict[str, torch.Tensor], style_features: Dict[str, torch.Tensor], content_weight: float, style_weight: float)` | Combines content and style losses into a single scalar value for optimization.                                       |
